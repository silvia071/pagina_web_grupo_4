...
export function guardarProductos() {
  ...
  *
*/
// data.js — local + Fake Store API (ES) con caché

// ------- Datos locales (tus originales) -------
const productosOriginales = [
  {
    id: "101",
    nombre: "Bowl de acero inoxidable",
  ...
  },
  {
    ...
    descripcion: "Práctico, decorativo y súper funcional.",
    detalle:
      "Canasto de ratán mediano, ideal para organizar y decorar cualquier espacio de tu hogar. Perfecto para almacenar ropa, juguetes, revistas u otros objetos, manteniendo todo en orden con estilo. Su diseño natural y elegante se adapta a cualquier ambiente.",
    categoria: "organizadores",
    precio: 20000,
    stock: 12,
    img: "assets/img/Canasto-ratan.jpg",
  },
  {
    ...
    descripcion: "Jabonera lisa - Varios colores.",
    detalle:
      "Jabonera lisa disponible en varios colores, perfecta para mantener tu jabón seco y al alcance de la mano en el baño o la cocina. Su diseño sencillo y funcional se adapta a cualquier estilo de decoración, añadiendo un toque de color y practicidad a tu espacio.",
    categoria: "baño",
    precio: 5000,
    stock: 12,
    img: "assets/img/Jabonera.jpg",
  },
  {
    ...
    descripcion: "Banqueta plástica reforzada apilable.",
    detalle:
      "Banqueta plástica reforzada apilable, ideal para uso en interiores y exteriores. Su diseño resistente soporta hasta 150 kg, proporcionando una solución práctica y cómoda para sentarse o alcanzar objetos en lugares altos. Fácil de limpiar y almacenar gracias a su capacidad de apilamiento.",
    categoria: "jardín",
    precio: 32000,
    stock: 12,
    img: "assets/img/Banqueta-reforzada.png",
  },
  {
    ...
    descripcion: "Set de cuchillos con soporte.",
    detalle:
      "Set de cuchillos de cocina que incluye 6 piezas esenciales para diversas tareas culinarias, junto con un soporte para mantenerlos organizados y accesibles. Cada cuchillo está diseñado con hojas afiladas y mangos ergonómicos para un manejo cómodo y seguro. Ideal para cortar, picar y filetear con precisión en la cocina.",
    categoria: "cocina",
    precio: 28000,
    stock: 6,
    img: "assets/img/set-cuchillos.jpg",
  ...
  },
  {
    ...
    descripcion: "Ø 28 cm, apta exterior.",
    detalle:
      "Maceta de cemento mediana con un diámetro de 28 cm, apta para uso en exteriores. Su diseño robusto y moderno es ideal para plantar flores, suculentas u otras plantas, añadiendo un toque de estilo a tu jardín, balcón o terraza. Resistente a las inclemencias del tiempo, esta maceta es perfecta para cualquier espacio al aire libre.",
    categoria: "jardín",
    precio: 22000,
    stock: 9,
    img: "assets/img/maceta-cemento.png",
...
// ------- Modo local (compatible con tu código actual) -------
function cargarProductosLocal() {
  const guardados = localStorage.getItem("productosStock");
  if (guardados) return JSON.parse(guardados);
  return productosOriginales;
}
export const productos = cargarProductosLocal();

export function guardarProductos() {
  localStorage.setItem("productosStock", JSON.stringify(productos));
}

// ------- Modo API (Fake Store) con categorías en ES + caché -------
...
function mapFakeStoreItem(x) {
  return {
    id: String(x.id),
    nombre: x.title,
    descripcion: x.description,
    detalle: x.description,
    categoria: catES[x.category] || x.category,
    precio: Number(x.price),
    stock: null, // Fake Store no trae stock real
    img: x.image,
  };
}

// Trae productos desde API (con caché en localStorage) y opcionalmente filtra por categoría ES
export async function getProductos({
  categoria = "todas",
  cacheMin = 60,
} = {}) {
  const cacheKey = "productosAPI_cache";
  const metaKey = "productosAPI_meta";

  // leer caché
  const meta = JSON.parse(localStorage.getItem(metaKey) || "{}");
  const ahora = Date.now();
  const vigente = meta.ts && ahora - meta.ts < cacheMin * 60 * 1000;

  if (vigente) {
    try {
      const cache = JSON.parse(localStorage.getItem(cacheKey) || "[]");
      return filtrarPorCategoria(cache, categoria);
    } catch {
      // si falla parseo, seguimos a red
    }
  }

  // pedir a la API
  const data = await apiGet("/products"); // todos
  const mapped = data.map(mapFakeStoreItem);

  // guardar caché
  localStorage.setItem(cacheKey, JSON.stringify(mapped));
  localStorage.setItem(metaKey, JSON.stringify({ ts: ahora }));

  return filtrarPorCategoria(mapped, categoria);
}

function filtrarPorCategoria(lista, categoria) {
  if (!categoria || categoria === "todas") return lista;
  return lista.filter((p) => p.categoria === categoria);
}

// Helper para traer categorías en español desde Fake Store
export async function getCategoriasES() {
  const catsEN = await apiGet("/products/categories"); // ["electronics","jewelery",...]
  // mapeo directo a ES
  return catsEN.map((c) => catES[c] || c);
}
